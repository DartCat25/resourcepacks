...
void main() {
    ...
    {
        case 1:
            vec4 glPos = ProjMat * ModelViewMat * vec4(0, 0, num * 0.5 , 1);
            float depth = (glPos.z / glPos.w) * 0.5 + 0.5;
            gl_FragDepth = depth;
        break;
        case 2:
            glPos = ProjMat * ModelViewMat * vec4(0, 0, min(num/helpCoord.y * 0.5, 1000)-1, 1);
            depth = (glPos.z / glPos.w) * 0.5 + 0.5;
            gl_FragDepth = depth;
            color *= tint;
            //if (helpCoord.y <= 1.0/floor(num * 0.5 + 1))
                //discard;
        break;
        case 3:
            int maxHP = num;
            while (maxHP > 0)
            {
                color = sample(color, uv, vec2(19 - p + int(num > 9) * 6, 135), vec2(maxHP % 10 * 8, 113), vec2(5, 7), tint);
                maxHP /= 10;
                p += 6;
            }
        break;
        case 4:
            ...
            while (HP > 0)
            {
                color = sample(color, uv, vec2(7 - p, 135), vec2(HP % 10 * 8, 113), vec2(5, 7), tint);
                HP /= 10;
                p += 6;
            }
        break;
        case 5:
            ...
            while (armorHP > 0)
            {
                color = sample(color, uv, vec2(7 - p, 142), vec2(armorHP % 10 * 8, 113), vec2(5, 7), tint);
                armorHP /= 10;
                p += 6;
            }
        break;
        case 6:
            ...
            while (airHP > 0)
            {
                color = sample(color, uv, vec2(10 - p, 151), vec2(airHP % 10 * 8, 113), vec2(5, 7), tint);
                airHP /= 10;
                p += 6;
            }
        break;
